##python script/run_single_CNN_flu.py -c ./config/single_task/CNN/fluorescence_CNN.yaml --seed 0
#python script/run_single_CNN_flu.py -c ./config/single_task/CNN/fluorescence_CNN_ourembedding.yaml --seed 0
...
from peer import protbert, util, flip,ProteinConvolutionalNetwork_ourembedding


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("-c", "--config", help="yaml configuration file",
                        default="config/single_task/CNN/fluorescence_CNN.yaml")
...
def build_solver(cfg, logger):
    ...
    model = R.search("ProteinConvolutionalNetwork_ourembedding")(input_dim=cfg['task']['model']['input_dim'],hidden_dims=cfg['task']['model']['hidden_dims'],kernel_size=cfg['task']['model']['kernel_size'], padding=cfg['task']['model']['padding'], readout="max")
    task = tasks.PropertyPrediction(model, task=_dataset.tasks,criterion=cfg.task.criterion, metric=cfg.task.metric,normalization=cfg.task.normalization, num_mlp_layer=cfg.task.num_mlp_layer)
    
    # fix the pre-trained encoder if specified
    fix_encoder = cfg.get("fix_encoder", False)
    fix_encoder2 = cfg.get("fix_encoder2", False)
    if fix_encoder:
        for p in task.model.parameters(): #just fix all the pre-trained model's weight
