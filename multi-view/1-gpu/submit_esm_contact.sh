...
#SBATCH --job-name contact-1gpu			#Nameof the job
#SBATCH --output slurm-contact1g-%j.out                #Output file name
#SBATCH -e slurm-contact1g-%j.err                      #error file name
...
#SBATCH --time=15:00:00					#Time required for the Jupyter job
...
python3 -m torch.distributed.launch --nproc_per_node=1 script-peer/run_single_ESM_contact.py -c ./config/single_task/ESM/contact_ESM.yaml --seed 0
#load model
