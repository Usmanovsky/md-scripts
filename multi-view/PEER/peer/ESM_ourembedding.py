...
@R.register("models.ESM_ourembedding")
class EvolutionaryScaleModeling_ourembedding(nn.Module, core.Configurable):
    """
    ...
    max_input_length = 500 #1024 - 2
    
    def __init__(self, path, model="ESM-1b", readout="mean",our_representation_dim=d_model):
        super(EvolutionaryScaleModeling_ourembedding, self).__init__()
        ...
        self.output_dim = self.output_dim[model]+our_representation_dim
        self.model = _model
    ...
    def forward(self, graph, input, all_loss=None, metric=None):
        ...
        """
        input = self.mapping[graph.residue_type]
        '''
        current_device = input.get_device()
        if current_device !=-1:
            input2 = torch.tensor([map2ourid(i.item()) for i in graph.residue_type]).cuda(current_device)
        else:
            input2 = torch.tensor([map2ourid(i.item()) for i in graph.residue_type]).cpu()
        '''
        input2 = torch.tensor([map2ourid(i.item()) for i in graph.residue_type]).to(self.device)
        ...
        residue_feature1 = output["representations"][33]  #(batch_size,residue_length+2,1280)
        
        input2 = functional.variadic_to_padded(input2, size, value=0)[0]# in Transformer_representation_embedding we used 0 for padding 
        '''
        if current_device !=-1:
            our_representation=model_seq.cuda(current_device)(input2)
        else:
            our_representation=model_seq(input2)
        '''
        our_representation=model_seq.to(self.device)(input2)
        
        if self.alphabet.prepend_bos:
           our_representation = torch.cat([torch.zeros([our_representation.size()[0],1,our_representation.size()[-1]]).to(self.device),our_representation],dim=1)#[batch_size,residue_len,128]
        
        if self.alphabet.append_eos:
           our_representation = torch.cat([our_representation,torch.zeros([our_representation.size()[0],1,our_representation.size()[-1]]).to(self.device)],dim=1)#[batch_size,residue_len,128]
        
        residue_feature1 = functional.padded_to_variadic(residue_feature1, size_ext) #batch_size*(residue_len+2)
        residue_feature2=functional.padded_to_variadic(our_representation,size_ext)
