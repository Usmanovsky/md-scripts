#python script/run_single_ESM_contact.py -c ./config/single_task/ESM/contact_ESM.yaml --seed 0
#python script/run_single_ESM_contact.py -c ./config/single_task/ESM/contact_ESM_fix.yaml --seed 0
...
from peer import protbert, util, flip,ESM_ourembedding
...
def build_solver(cfg, logger):
    ...
    #model = EvolutionaryScaleModeling_ourembedding(path="~/scratch/esm-model-weights/",model="ESM-1b",readout="mean")
    model = R.search("ESM_ourembedding")(path=cfg.task.model.path,model=cfg.task.model.model)
    
    task = tasks.ContactPrediction(model,max_length=cfg.task.max_length,random_truncate=True, threshold=cfg.task.threshold, gap=cfg.task.gap, criterion=cfg.task.criterion,metric=cfg.task.metric, num_mlp_layer=cfg.task.num_mlp_layer)
