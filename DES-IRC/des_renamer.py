...
if __name__ == "__main__":  # Accept command line inputs for  ligpargen files only if script is called directly.
    ...
    if len(sys.argv[3]) > 1:
        molnumx = int(sys.argv[3])  # Number of molecules for Compound A
    else:
        molnumx = 50
        
                
    if len(sys.argv[4]) > 1:
        molnumy = int(sys.argv[4])  # Number of molecules for Compound B
    else:
        molnumy = 50
...
#             print(res2)
    ...
    #             os.mkdir(f"X")        
                ...
                for number in range(1,4):  # Copying mdp and topology files.               
                    # mdp_path = des_name + str(number)  # pick this if you want your mdp files to be named like Dea-Men111.mdp
                    mdp_path = str(number)  # pick this if you want your mdp files to be named like 1.mdp
                ...
                os.system(f"sed -i 's/{title}/{des_name}/g' {top_path}")
