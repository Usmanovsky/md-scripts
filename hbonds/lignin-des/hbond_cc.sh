...
do
	...
	then 
        ...
        then
                { echo 2; echo 3; echo q; } | gmx_gpu make_ndx -f $name$c.gro -o $name$h.ndx
                a=$x
                { echo 5; echo 4; echo q; } | gmx_gpu hbond -f $name$f.trr -s $name$f.tpr -n $name$h.ndx -ac $l-$name-$a.xvg -num $n-$name-$a.xvg -ang $ang-$name-$a.xvg -dist $d-$name-$a.xvg > $a-$name-$type1.log
                /data/ul*/Sc*/A*/unxvg2txt.sh '.'
                mover ./$h-$name-$a
