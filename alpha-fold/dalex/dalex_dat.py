...
for res in residues:
    ...
    ress = res + file.stem[-1]
    fobject.plot(show=False).write_image(f"{plot_path}/{ress}.png", format='png', engine='auto')
    print('\n \n \n')
...
#    fobject = exp.model_fairness(protected, privileged)
#    print(fobject.fairness_check())
#    fobject.plot(show=False).write_image(f"{ss_name[ss.index(s)]}.png", format='png', engine='auto')
#    print('\n \n \n')
