...
def hnum(acceptors,hydrogens, comp_A, comp_B, comp_C, z, limit, mols,water=False):
    ...
    hb.run(start=0, stop=20000, step=100)
...
def hsolve(z, limit, comp_A, comp_B, comp_C):
        ...
        if z < limit:  # Bulk DES
            ...
            # des acceptor, water donor            
            acceptors_int = f"(resname {comp_A} or resname {comp_B}) and (name O* N*) and prop z >= {lower} and prop z <= {upper}"    
            hydrogens_int = f"(resname {comp_C} and name HW*) and prop z >= {lower} and prop z <= {upper}"
            ...
            # water acceptor, des donor
            acceptors_inta = f"(resname {comp_C} and name OW*) and prop z >= {lower} and prop z <= {upper}"
            hydrogens_inta = f"(resname {comp_A} or resname {comp_B}) and (name H* and bonded name O* N*) and prop z >= {lower} and prop z <= {upper}"
            molecules_inta = box.select_atoms(f"(resname {comp_B} and name {atomsB}) or (resname {comp_A} and name {atomsA}) and prop z >= {lower} and prop z <= {upper}").n_atoms
            water_inta = box.select_atoms(f"(resname {comp_C} and name {atomsC}) and prop z >= {lower} and prop z <= {upper}").n_atoms
            print(f"There are {molecules_int} {comp_A} plus {comp_B} plus {water_int} {comp_C}  molecules in Interface DES-Water zone {lower}-{upper}")
            # print("Interface DES-Water zone")
            num2a = hnum(acceptors_inta, hydrogens_inta, comp_A,comp_B, comp_C, z, limit, molecules_inta)
            ...
            num2_total = num2 + num2a
            hnumber.append(num1)
            hnumber.append(num2_total)
            hnumber.append(num3)
            print("DES-Water, Water-DES", num2 + num2a)
            return hnumber
...
for file in pathway.glob(path):
    ...
    if os.path.isdir(file):        
        ...
        fig1.savefig(f'{file}/{des_name}-hnumbar_deswaterfix.png', dpi=fig1.dpi, bbox_inches='tight')
        plt.show()
