...
"""
11/2/2021
In this update, I fixed a bug that overwrote gro files that do not have
_md in their names when box_rescaler.sh is called. Improved the if 
statements for gro1_name and gro2_name so molar_ratio is correct even if
gro1/gro2 has _npt or _md in their names. 
...
python3 gro_merger_v8.py Dea-Men11_md.gro Thy-Lid11.gro folder-path-to-save
"""
...
if gro1.__contains__("/") and gro1.__contains__("_"):
    gro1_name = gro1.split('/')[-1].split('_')[0]
    unscaled_groname = gro1.split("/")[-1]
elif gro1.__contains__("_"):
     gro1_name = gro1.split('_')[0]
     unscaled_groname = gro1
else:
    gro1_name = gro1[0:-4]
    unscaled_groname = gro1_name + ".gro"

if gro2.__contains__("/") and gro2.__contains__("_"):
    gro2_name = gro2.split('/')[-1].split('_')[0]
    gro2_untouched = gro2.split('/')[-1]
elif gro2.__contains__("_"):
     gro2_name = gro2.split('_')[0]
     gro2_untouched = gro2
else:
    gro2_name = gro2[0:-4]
    gro2_untouched = gro2
...
print(f"gro1_name: {gro1_name}, gro2_name: {gro2_name}, unscaled_groname: {unscaled_groname}, gro2_untouched: {gro2_untouched}")
...
if des_rescale[1].__contains__('_'):
    molar_ratio = des_rescale[1][3:5]
else:
    molar_ratio = des_rescale[1][3:]


print(compoundA, compoundB)
rescaled_gro1_name = gro1_name + "_rescaled.gro"
...
try:
    os.system(f"./box_rescaler-lcc.sh {compoundA} {compoundB} {molar_ratio} {x_rescale} {y_rescale} {unscaled_groname};")
except:
    print("LCC box_rescaling did not work. Now trying LabGPU")
    os.system(f"./box_rescaler-gpu.sh {compoundA} {compoundB} {molar_ratio} {x_rescale} {y_rescale} {unscaled_groname};")
...
try:
    os.system(f"./mol_shifter-lcc.sh {compound1} {compound2} {molar_ratio} {distance_zshift} {gro2_untouched};")
except:
    print("LCC did not work. Now trying LabGPU")
    os.system(f"./mol_shifter-gpu.sh {compound1} {compound2} {molar_ratio} {distance_zshift} {gro2_untouched};")
